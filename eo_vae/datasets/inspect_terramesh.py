...
CHANNEL_NAMES = ['B01', 'B02', 'B03', 'B04', 'B05', 'B06', 'B07', 'B08', 'B8A', 'B09', 'B11', 'B12']
...
def check_local_data():
    ...
    for tar_path in all_files:
        if total_processed >= SAMPLES_TO_CHECK: break
            ...
            for member in tar:
                ...
                if member.name.endswith(".zarr.zip"):
                    ...
                    with tempfile.NamedTemporaryFile(suffix='.zip') as tmp:
                        ...
                        with zarr.open(store=zarr.ZipStore(tmp.name, mode='r'), mode='r') as z:
                            # --- 1. Date Parsing ---
                            ts_str = None
                            if 'time' in z.attrs: ts_str = z.attrs['time']
                            elif 'time' in z: 
                                t_val = z['time'][...]
                                ts_str = str(t_val[0]) if (isinstance(t_val, (np.ndarray, list)) and len(t_val) > 0) else str(t_val)
                            
                            if not ts_str: continue

                            clean_ts = str(ts_str).strip(" []'")
                            if clean_ts.isdigit():
                                ts = datetime.datetime.fromtimestamp(int(clean_ts) / 1e9)
    ...
    print(f"{'Channel':<5} | {'Pre Mean':<10} {'Post Mean':<10} | {'Pre Min':<10} {'Post Min':<10} | {'Pre Std':<10} {'Post Std':<10} | {'Diff (Mean)':<10}")
    print("-" * 90)
    for c in range(12):
        diff = mean_post[c] - mean_pre[c]
        print(f"{CHANNEL_NAMES[c]:<5} | {mean_pre[c]:<10.1f} {mean_post[c]:<10.1f} | {stats_pre.min[c]:<10.1f} {stats_post.min[c]:<10.1f} | {std_pre[c]:<10.1f} {std_post[c]:<10.1f} | {diff:<10.1f}")
    ...
    for c in range(12):
        ...
        # Plot
        ax.plot(bin_centers, dens_pre, color='blue', label='Pre-2022', alpha=0.8, linewidth=2)
        ax.plot(bin_centers, dens_post, color='orange', label='Post-2022', alpha=0.8, linewidth=2)
        
        # Add a shifted version of Post to see if they align
        # Heuristic: Shift Post by +1000 to see if it matches Pre
        ax.plot(bin_centers + 1000, dens_post, color='green', linestyle=':', label='Post + 1000', alpha=0.6)
    ...
    plt.suptitle(f"Sentinel-2 Distribution Shift Analysis\nSample Count: Pre={stats_pre.count}, Post={stats_post.count}", fontsize=16)
    plt.tight_layout()
