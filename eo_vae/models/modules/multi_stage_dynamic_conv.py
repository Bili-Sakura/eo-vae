...
class DynamicConvBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm = nn.GroupNorm(min(32, embed_dim), embed_dim) if use_norm else nn.Identity()
    ...
    def forward(self, x: Tensor, waves_embedded: Tensor) -> Tensor:
        ...
        dynamic_weight = dynamic_weight.expand(-1, self.embed_dim, -1, -1)  # depthwise-ish
...
class WavelengthAdaptiveWeightGenerator(nn.Module):
    ...
    def forward(self, waves: Tensor) -> tuple[Tensor, Tensor]:
        ...
        tokens = torch.cat([
            self.weight_tokens.unsqueeze(0).expand(1, -1, -1),
            waves.unsqueeze(0)
        ], dim=1)  # [1, wt_num + num_wv, wv_planes]
...
class MultiStageDynamicDecoder(nn.Module):
    ...
    def forward(self, x: Tensor, wvs: Tensor) -> Tensor:
        ...
        # Stage 3: Final wavelength-specific projection
        weights, biases = self.final_generator(waves)  # [num_wv, K*K*embed_dim], [num_wv, 1]
        
        # Reshape weights for conv2d
        dynamic_weight = weights.view(
            num_wv, self.kernel_size, self.kernel_size, self.embed_dim
        )
        dynamic_weight = dynamic_weight.permute([0, 3, 1, 2])  # [num_wv, embed_dim, K, K]
    ...
    def get_distillation_weight(self, wvs_microns: Tensor) -> tuple[Tensor, Tensor | None]:
        ...
        """
        # Compute wavelength embeddings
        waves = get_1d_sincos_pos_embed_from_grid_torch(self.wv_planes, wvs_microns * 1000)
        waves = self.fclayer(waves)  # [num_wv, wv_planes]
...
class ProgressiveMultiStageDynamicDecoder(nn.Module):
    ...
    def forward(self, x: Tensor, wvs: Tensor) -> Tensor:
        ...
        Returns:
            Reconstructed multi-spectral image [B, num_wavelengths, H, W]
        """
        # Store input for potential skip
        x_in = x
    ...
    def get_distillation_weight(self, wvs_microns: Tensor) -> tuple[Tensor, Tensor | None]:
        ...
        waves_attn = final_stage.wave_norm(waves_attn.squeeze(0) + waves)  # [num_wv, wv_planes]
...
class DecoderStage(nn.Module):
    ...
    def forward(self, x: Tensor, waves: Tensor) -> Tensor:
        ...
        waves_attn = self.wave_norm(waves_attn.squeeze(0) + waves)  # [num_wv, wv_planes]
