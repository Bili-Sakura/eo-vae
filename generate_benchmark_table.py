...
def generate_latex_table(results, metadata, output_path, custom_caption=None):
    ...
    lines.append("\\toprule")
    lines.append("\\textbf{Model} & \\textbf{Time (ms)} & \\textbf{Throughput} & \\textbf{Memory (GB)} & \\textbf{Params (M)} \\\\")
    lines.append("               &                     & (imgs/sec)           &                      & Total (Diffusion) \\\\")
...
def generate_latex_detailed_table(results, metadata, output_path):
    ...
    caption += "Encode and Decode times are for VAE operations (zero for pixel-space models)."
    ...
    lines.append("\\toprule")
    lines.append("\\textbf{Model} & \\textbf{Encode} & \\textbf{SR Forward} & \\textbf{Decode} & \\textbf{Total} & \\textbf{Speedup} \\\\")
...
def print_summary_stats(results):
    ...
    if baseline:
        ...
        for latent in latent_models:
            ...
            print(f"  - Memory ratio: {mem_ratio:.2f}x {'less' if mem_ratio > 1 else 'more'}")
            print(f"  - Timing breakdown:")
            print(f"      Encode: {latent['timing_ms']['encode']:.2f} ms ({latent['timing_ms']['encode']/latent_time*100:.1f}%)")
            print(f"      SR:     {latent['timing_ms']['sr_forward']:.2f} ms ({latent['timing_ms']['sr_forward']/latent_time*100:.1f}%)")
            print(f"      Decode: {latent['timing_ms']['decode']:.2f} ms ({latent['timing_ms']['decode']/latent_time*100:.1f}%)")
    
    # Compare latent models to each other
    if len(latent_models) >= 2:
        print(f"\nLatent Model Comparison:")
        m1, m2 = latent_models[0], latent_models[1]
        ratio = m1['timing_ms']['total'] / m2['timing_ms']['total'] if m2['timing_ms']['total'] > 0 else 0
        print(f"  {m1['name']} vs {m2['name']}: {ratio:.2f}x")
        print(f"  (Values close to 1.0 indicate similar performance)")
