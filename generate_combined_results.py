...
def generate_compact_latex_table(combined_results, output_path):
    ...
    all_modalities = sorted(set(mod for model_data in combined_results.values() for mod in model_data.keys()))
    ...
    for modality in all_modalities:
        for metric in get_metrics_for_modality(modality):
            arrow = r'$\downarrow$' if metric in ['RMSE', 'SAM', 'NDVI_MAE'] else r'$\uparrow$'
...
def main():
    parser = argparse.ArgumentParser(description='Combine evaluation results and generate LaTeX table')
    ...
    latex_table = generate_latex_table(combined, latex_path)
    ...
    for model_name, model_data in combined.items():
        ...
        for modality in sorted(model_data.keys()):
            metrics_str = ", ".join([f"{k}: {v:.4f}" for k, v in model_data[modality].items()])
