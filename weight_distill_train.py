...
def load_teacher_weights(ckpt_path: str) -> dict[str, Tensor]:
    ...
    print(f"  Encoder conv_in bias shape: {enc_bias.shape if enc_bias is not None else 'None'}")
    print(f"  Decoder conv_out weight shape: {dec_weight.shape}")
    print(f"  Decoder conv_out bias shape: {dec_bias.shape if dec_bias is not None else 'None'}")
...
class DistillationModule(LightningModule):
    ...
    def __init__(
        ...
        self.register_buffer(
            'rgb_wvs',
            torch.tensor(config.rgb_wavelengths, dtype=torch.float32)
        )
    ...
    def compute_distillation_loss(self) -> tuple[Tensor, dict[str, Tensor]]:
        ...
        if self.encoder.use_dynamic_ops:
            ...
            with torch.no_grad():
                ...
                logs['enc_weight_max_err'] = (student_enc_w - self.teacher_enc_w).abs().max()
        ...
        if self.decoder.use_dynamic_ops:
            ...
            # Get dynamic weights for RGB wavelengths
            student_dec_w, student_dec_b = conv_out.get_distillation_weight(self.rgb_wvs)
            ...
            with torch.no_grad():
                ...
                logs['dec_weight_max_err'] = (student_dec_w - self.teacher_dec_w).abs().max()
...
def save_distillation_checkpoint(
    ...
    checkpoint = {
        ...
        'encoder_conv_in_state_dict': (
            ...
            if module.encoder.use_dynamic_ops else None
        ),
        'decoder_conv_out_state_dict': (
            module.decoder.conv_out.state_dict()
            if module.decoder.use_dynamic_ops else None
        ),
...
def load_distilled_checkpoint(
    ...
    if encoder.use_dynamic_ops and ckpt.get('encoder_conv_in_state_dict'):
        missing, unexpected = encoder.conv_in.load_state_dict(
            ckpt['encoder_conv_in_state_dict'],
            strict=strict,
        )
        print(f"Loaded encoder.conv_in: {len(missing)} missing, {len(unexpected)} unexpected")
    ...
    if decoder.use_dynamic_ops and ckpt.get('decoder_conv_out_state_dict'):
        missing, unexpected = decoder.conv_out.load_state_dict(
            ckpt['decoder_conv_out_state_dict'],
            strict=strict,
        )
        print(f"Loaded decoder.conv_out: {len(missing)} missing, {len(unexpected)} unexpected")
...
def run_distillation(
    ...
    if not encoder.use_dynamic_ops and not decoder.use_dynamic_ops:
        raise ValueError("Neither encoder nor decoder uses dynamic ops. Nothing to distill!")
    ...
    datamodule = DummyDataModule(
        train_size=distill_config.max_steps,
        val_size=100,
    )
...
def main():
    ...
    parser.add_argument(
        '--teacher-ckpt', type=str, required=True,
    ...
    parser.add_argument(
        '--val-every', type=int, default=500,
